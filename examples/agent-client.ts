/**
 * x402-movies — agent client example.
 *
...
async function main(): Promise<void> {
  ...
  const unpaid = await fetch(`${BASE_URL}/search?q=blade%20runner`);
  ...
  {
    const res = await payFetch(`${BASE_URL}/search?q=blade%20runner`);
    if (!res.ok) {
      console.error(`GET /search failed: ${res.status}`, await res.text());
      process.exit(1);
    }
    console.log("=== GET /search ($0.001) ===");
  ...
  {
    const res = await payFetch(`${BASE_URL}/movie/524916`);
    if (!res.ok) {
      console.error(`GET /movie/:id failed: ${res.status}`, await res.text());
      process.exit(1);
    }
    console.log("=== GET /movie/:id ($0.001) ===");
    console.log(JSON.stringify(await res.json(), null, 2));
  ...
  {
    const res = await payFetch(`${BASE_URL}/recommendations/524916?limit=3`);
    if (!res.ok) {
      console.error(`GET /recommendations/:id failed: ${res.status}`, await res.text());
      process.exit(1);
    }
    console.log("=== GET /recommendations/:id ($0.002) ===");
    console.log(JSON.stringify(await res.json(), null, 2));
