# For AI agents — x402-movies

`x402-movies` is a catalogue an agent can query without ever provisioning a key.
Three routes, priced from $0.001, each returning its artifact in the same
response. The recommendations route is the interesting one: it re-ranks the
upstream's related titles and tells you *why* each one placed where it did.
...
| [`skill.md`](https://github.com/nirholas/x402-movies/blob/main/skill.md) | The agent-facing skill file: prose an LLM can read directly to learn the endpoints, parameters, and payment flow. |
| [`openapi.json`](https://github.com/nirholas/x402-movies/blob/main/openapi.json) | OpenAPI 3.1, including the 402 `PaymentRequirements` schema. |
...
const artifact = await (await payFetch("${BASE}/search?q=blade%20runner")).json();
```
...
[`examples/mcp-tool.md`](https://github.com/nirholas/x402-movies/blob/main/examples/mcp-tool.md)
